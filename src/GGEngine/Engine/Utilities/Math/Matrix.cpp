...
#include <GGEngine/Engine/Utilities/Math/Matrix.h>
#include <iomanip>
#include <stdexcept>
#include <string>
...
{
    EPP_ASSERT_M((i < 3), "Tried to get element out of matrix range");
    if (i < 3)
        return m[i];
    else
        throw std::out_of_range(std::string("Index out of array's range: ") + std::to_string(i));
}


const float_t& Matrix::operator()(uint8_t i, uint8_t j) const
{
    EPP_ASSERT_M((i < 3 || j < 3), "Tried to get element out of matrix range");
    if (i < 3 && j < 3)
        return m[i][j];
    else
        throw std::out_of_range(std::string("Index out of array's range: ") + std::to_string(i) + std::to_string(j));
}
